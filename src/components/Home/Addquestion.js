...
export default function Addquestion(){
    ...
    return (
        <div className={darkMode ? "MainDark AddquestinHome":"Main AddquestinHome"} style={{gap:"8px",cursor:"pointer"}} >
            <section className="flexPro" style={{gap:"12px"}} onClick={openModal}>
           
            {user && <Avatar src={user?.photo} alt="profile_img" />}
        {name && <main id="ProfileIcon"style={{width:"40px",height:"40px"}}>{name.charAt(0).toUpperCase()}</main>}
            ...
            <section className="flexjust">
              ...
              {modalOpen && <Addpost  closeModal={() => setModalOpen(false)}/>}
